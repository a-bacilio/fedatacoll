...
function QuestionsTable ({data, refetch}){
  ...
  return (
    <table className='w-full max-w-3xl p-0 mx-auto text-xs border-2 border-white sm:text-base sm:p-5'>
          ...
          ? (
              data.map((item,key) => (
                <tr key={"fila"+key}>
                  ...
                  <td className='px-2 py-1 text-center border-2 border-white' >{item.labelText}</td>
                  <td className='px-2 py-1 text-center border-2 border-white' >{item.questionType}</td>
                  <td className='px-2 py-1 text-center border-2 border-white' >
                    <span className='flex flex-row justify-center w-full text-fuchsia-500'>
                      <Link to={`/project/question/edit/${item.questionType}/${item._id}`}><BsFillPencilFill className='cursor-pointer sm:w-8 sm:px-2'/></Link>
                      {key<=0?null:<ButtonUp projectid={projectid} order={parseInt(key)} refetch={refetch} />}
                      {key>=data.length-1?null:<ButtonDown projectid={projectid} order={parseInt(key)} refetch={refetch} />}
                      <ButtonDelete projectId={projectid} questionId={item._id} refetch={refetch} />
                    </span>
                  </td>
                </tr> 
                )
        ...
        :<tr><td colSpan={5}><NotificationSpan className="px-2">No hay preguntas</NotificationSpan></td></tr>
          }
        </tbody>
      </table>
  )
